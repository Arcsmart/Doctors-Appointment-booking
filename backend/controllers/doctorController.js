...
const changeAvilabilty = async (req, res) => {
  try {
    ...
    res.json({ sucess: true, message: "Avilablity changed" });
  } catch (error) {
    console.log(error);
...
const appointmentCancel = async (req, res) => {
  try {
    ...
    if (appointmentData && appointmentData.docId === docId) {
      ...
      return res.json({ success: false, message: "Cancelation Faild" });
    }
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
...
const doctorDashboard = async (req, res) => {
  try {
    ...
    const patents = [];
    appointments.map((item) => {
      if (!patents.includes(item.userId)) {
        patents.push(item.userId);
      }
    });
    const dashData = {
      earnings,
...
// API to update doctor profile data doctor panel
