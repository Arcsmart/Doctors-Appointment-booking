...
const Navbar = () => {
  ...
  const [showMenu, setShowMenu] = useState(false); // Changed initial state to false (assuming menu is initially closed)
...
WeCare
      ...
      <div
        className={` ${
          showMenu ? "fixed w-full h-full" : "h-0 w-0" // Added h-full for better coverage
        } md:hidden right-0 top-0 bottom-0 z-40 sticky top-0 z-30  overflow-hidden bg-white transition-all`}
      >
        <div className="flex items-center justify-between px-5 py-6">
          {/* Use the logo image from the import for better consistency */}
